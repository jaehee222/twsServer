...
public class UserService {
    ...
    // 비밀번호 변경
    public boolean changePassword(String userId, UserDto userDto, String pwInfo) {
        boolean isCorrect = false;

        if (pwInfo == "findPw") {   // 비밀번호 찾기
            isCorrect = true;
        } else if (pwInfo == "changeInfo") {    // 비밀번호 변경
            isCorrect = login(userId, userDto.getPassword());
        }
        if (isCorrect) {
            UserEntity user = userRepository.findByUserId(userId);
